...
- premaster secret: 这是 48B 的 blob 数据. 它能和 client & server random 通过 pseudorandom(PRF) 一起生成
session key.
...
例如: ECDH-ECDSA-AES256-GCM-SHA384 代表, ECDH 表示使用 ECDH 算法进行秘钥交换, 
ECDSA 表示证书签名算法, AES256-GCM 对称加密算法, SHA384 数据签名校验.
```
...
1. Client 发送 ClientHello 信息, 包含了客户端支持的最高 TLS 协议版本, random num, cipher suite. 如果客户端使用的
resumed handshake, 那么这里发送的就是 Session ID. 以及其他的扩展数据.

2. Server 发送 ServerHello 信息, 这里 server 根据 client 发送过来的相关信息, 采取不同的策略, 同样还会发送和
client 端匹配的TLS最高版本信息, cipher suite 和 自己产生的 random num. 并且, 这里会产生该次连接独一无二的 
Session ID. 以及其他的扩展数据
...
+ `client random-num` 生成 SessionKey. 这个标志着在 client 端, TLS 过程已经接近尾声.

6. Server 端进行的 ChangeCipherSpec 与 client 进行的差不多, 通过使用 private key 解密 client 传过来的
pre-master secret, 然后生成 SessionKey. 后面再通过一次验证, 使用 SessionKey 加密 server finished, 发送给 
client, 观察能否成功解密, 来表示最终的 TLS 完成.
