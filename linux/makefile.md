...
通常, make 会把其要执行的命令行在命令执行钱输出到屏幕上. 当使用 `@` 字符在命令行前, 那么, 这个命令将不被 make
显示出来.
...
如果 make 执行时, 带人参数 `-n` 或者 `--just-print`, 那么只是显示命令, 但不执行命令.

而 make 参数 `-s` 或 `--slient` 或 `--quite` 则是全面禁止命令的显示.


## 使用变量

变量在声明时候需要给予初始值, 而在使用时, 需要在黑变量名前添加 `$` 符号, 最好用小括号 `()` 或者大括号 `{}` 将变量名包括
