...
会话1的第一个操作获取该行的互斥锁(X). 会话2和3的操作都会导致重复键错误, 并且它们都请求该行的共享锁(S). 当会话1提交时, 它会释放
...
行上设置互斥锁而不是共享锁. 对重复的主键值采用独占 record lock. 对于重复的唯一键值, 采用独占 next-key lock.
...
1)表t当中没有 id=7 的记录. 根据原则1, 单位是next-key lock, 锁定范围是 (5, 10]. 
...
```
BEGIN;
UPDATE t SET d=d+1 WHERE id>=10 AND id<11;
```
...
2)使用优化1, 加记录锁定 (10,10), 由于是唯一索引, 无需继续搜索.
...
1)只是访问到了 id=5 的记录, 对记录5加记录锁.

#### 主键索引等值查询(不存在)

```
BEGIN;
UPDATE t SET d=d+1 WHERE id=11;
```
